...
void* ReadVertexData(GeometryChunkData dataType, GeometryChunkTypes chunkType, int numVertices) {
	...
	if (chunkType == GeometryChunkTypes::VTex0 || chunkType == GeometryChunkTypes::VTex1) {
		numElements = 2;
	}
...
void MeshGeometry::SetVertexTextureCoords(const vector<Vector3>& newTex) {
	cubeTexCoords = newTex;
}
