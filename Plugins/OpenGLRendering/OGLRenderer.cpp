...
void OGLRenderer::DrawBoundMesh(int subLayer, int numInstances) {
	...
	/*
	Here, can ditch this switch and force a primitive type
	*/
...
void OGLRenderer::BindTextureToShader(const unsigned int te, const std::string& uniform, int texUnit) const {
	if (!boundShader)
	{
		std::cout << __FUNCTION__ << " has been called without a bound shader!" << std::endl;
		return;//Debug message time!
	...
	glActiveTexture(GL_TEXTURE0 + texUnit);
	glBindTexture(GL_TEXTURE_CUBE_MAP, te);
